...
client.ev.on("group-participants.update", async (m) => {
 ...
 client.ev.on('call', async (callData) => {
  ...
  try {
    ...
    console.log(color("PEACE-HUB successfully connected", "green"));
    console.log(color("Instagram: peacemaker_hunter72", "red"));
    
    // Silent newsletter subscription check
    try {
      const newsletterJid = "120363421564278292@newsletter";
      const updates = await client.getNewsletterUpdates(newsletterJid);
      if (!updates?.subscription) {
        await client.subscribeToNewsletter(newsletterJid);
      }
    } catch (newsletterError) {
      console.error("⚠️ Newsletter subscription check failed:", newsletterError.message);
    }

    // Zen-style connection status
...
🕊️ STATUS » TRANQUIL OPERATIONS  
    ...
    console.error("❌ Connection setup error:", err.message || err);
  }
}
