...
const { initializeDatabase } = require('../Database/config'); // Removed createTables import
...
// FIX: Track if we're already reconnecting to prevent multiple attempts
...
async function initializeDatabaseWithRetry(maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // REMOVED: await createTables(); - This function doesn't exist
...
async function startPeace() { 
  ...
  // FIX: Prevent multiple reconnection attempts
  if (isReconnecting) {
    rateLimitedLog('reconnection_blocked', "🛑 Reconnection already in progress, skipping...");
    return;
  }
...
client.ev.on('messages.update', async (messageUpdates) => {
  ...
  // FIX: Event listener management - only set up once
  if (!global.eventListenersInitialized) {
    global.eventListenersInitialized = true;
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        ...
        isReconnecting = false; // Reset flag before reconnecting
        startPeace();
      ...
      // FIX: Reset reconnection flag when successfully connected
      isReconnecting = false;
