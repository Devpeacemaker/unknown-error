...
try {
  ...
  rateLimitedLog('settings_load', "😴 Settings object loaded");
...
client.ev.on('messages.update', async (messageUpdates) => {
  ...
  process.on("unhandledRejection", (reason, promise) => {
    ...
    rateLimitedLog('unhandled_rejection', `Unhandled Rejection at: ${promise}, reason: ${reason}`);
  });
  process.on("rejectionHandled", (promise) => {
    unhandledRejections.delete(promise);
  });
  process.on("Something went wrong", function (err) {
    rateLimitedLog('caught_exception', `Caught exception: ${err}`);
  });
  ...
  client.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        ...
        rateLimitedLog('unknown_reason', `Unknown DisconnectReason: ${reason}|${connection}`);
        startPeace();
      }
